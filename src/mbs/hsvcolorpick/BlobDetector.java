...
public class BlobDetector {
    ...
    private Scalar mMidBound = new Scalar(0);
    // Minimum contour area in percent for contours filtering
    private static double mMinContourArea = 0.0001;
    // Color radius for range checking in HSV color space
    public Scalar mColorRadius = new Scalar(30,4,4,0);
    ...
    public void setHsvColor(Scalar hsvColor) {
        ...
        mMidBound.val[0]	=  hsvColor.val[0];
        mMidBound.val[1]	=  hsvColor.val[1];
        mMidBound.val[2]	=  hsvColor.val[2];
...
//                param2 = mColor2;
    ...
    public void process(Mat rgbaImage, Scalar mMinBound, Scalar mMaxBound, double dilate1, double erode1) {
...
//        Imgproc.dilate(mMask, mDilatedMask, new Mat());
        ...
        Imgproc.erode(mMask, mDilatedMask, Imgproc.getStructuringElement(Imgproc.MORPH_ELLIPSE, new Size (2*erode1+1,2*erode1+1), new Point (erode1,erode1))); 
        Imgproc.dilate(mDilatedMask, mDilatedMask, Imgproc.getStructuringElement(Imgproc.MORPH_ELLIPSE, new Size (2*dilate1+1,2*dilate1+1), new Point (dilate1,dilate1)));
        Imgproc.dilate(mDilatedMask, mDilatedMask, Imgproc.getStructuringElement(Imgproc.MORPH_ELLIPSE, new Size (2*dilate1+1,2*dilate1+1), new Point (dilate1,dilate1)));
        Imgproc.erode(mDilatedMask, mDilatedMask, Imgproc.getStructuringElement(Imgproc.MORPH_ELLIPSE, new Size (2*erode1+1,2*erode1+1), new Point (erode1,erode1)));
    ...
    public double getHmid(){
		return mMidBound.val[0];
    	
    }
    
    public double getSmid(){
		return mMidBound.val[1];
    	
    }
    
    public double getVmid(){
		return mMidBound.val[2];
